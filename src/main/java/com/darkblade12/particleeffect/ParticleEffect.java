 ...
 * <li>@JamieSinn (hosting a jenkins server and documentation for com.darkblade.com.darkblade12.particleeffect)
 * </ul>
 * <p>
...
public enum ParticleEffect {
	...
	private static boolean isLongDistance(Location location, List<Player> players) {
		...
		for (Player player : players) {
			...
			if (!world.equals(playerLocation.getWorld().getName()) || playerLocation.distanceSquared(location) <
					65536) {
				continue;
			}
			return true;
	...
	private static boolean isDataCorrect(ParticleEffect effect, ParticleData data) {
		return ((effect == BLOCK_CRACK || effect == BLOCK_DUST) && data instanceof BlockData) || (effect == ITEM_CRACK
				&& data instanceof ItemData);
	}
	 ...
	 * {@link ParticleEffect#SPELL_MOB_AMBIENT}, {@link ParticleEffect#REDSTONE} and {@link ParticleEffect#NOTE}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 *
	 * @author DarkBlade12
	...
	public static final class ParticlePacket {
		 ...
		 */
		public ParticlePacket(ParticleEffect effect, Vector direction, float speed, boolean longDistance, ParticleData
				data) throws IllegalArgumentException {
		...
		public ParticlePacket(ParticleEffect effect, ParticleColor color, boolean longDistance) {
			...
			if (effect == ParticleEffect.REDSTONE && color instanceof OrdinaryColor && ((OrdinaryColor) color).getRed
					() == 0) {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				version = Integer.parseInt(
						PackageType.getServerVersion().substring(
								3, PackageType.getServerVersion().indexOf("_", 3)
						)
				);
				if (version > 7) {
					enumParticle = PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
				}
				Class<?> packetClass = PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles" :
				...
				playerConnection = ReflectionUtils.getField("EntityPlayer", PackageType.MINECRAFT_SERVER, false,
						"playerConnection");
				sendPacket = ReflectionUtils.getMethod(playerConnection.getType(), "sendPacket", PackageType
						.MINECRAFT_SERVER.getClass("Packet"));
			} catch (Exception exception) {
				throw new VersionIncompatibleException("Your current bukkit version seems to be incompatible with this" +
						" library", exception);
			}
			initialized = true;
		}
		 ...
		 */
		public void sendTo(Location center, Player player) throws PacketInstantiationException,
